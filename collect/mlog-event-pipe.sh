...
while read t line; do
    if grep -q --line-buffered 'BLOCKCHAIN WRITE BLOCK' <<< "$line"; then
        ...
        number=$(echo "$line" | sed -rn 's/.*block\.number=([0-9]+).*/\1/p')
        bhash=$(echo "$line" | sed -rn 's/.*block\.hash=([x0123456789abcdef]+).*/\1/p')

        # eg. 2019/01/09T21:07:31Z CANON 0x923fa3fa388812349bf
        echo "$t $bwstat $addr $delta_parent_time $uncles $number $bhash -> $f"
        echo "$t $bwstat $addr $delta_parent_time $uncles $number $bhash " >> "$f"
